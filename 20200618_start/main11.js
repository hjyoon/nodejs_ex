...
var app = http.createServer(function(request,response){
	...
	if(pathname === '/') {
		...
		request.on('end', function(data) {
			...
			client.query('insert into topic(title, description, created, author_id) values($1, $2, now(), $3) RETURNING id' , [title, description, 1], (err, res) => {
				if(err) {
					//console.log(err.stack)
					throw err;
				}
		...
		request.on('end', function(data) {
			...
			var description = post.description;
			client.query(`UPDATE topic SET title=$1, description=$2, author_id=$3 WHERE id=$4`, [title, description, 1, id], (err, res) => {
				if(err) {
					//console.log(err.stack)
					throw err;
				}
